                ...
                <div class="card-body">
                    <form action="{{ route('menu.update',$menus->id) }}" method="Post" enctype="multipart/form-data" >
                        ...
                        <div class="form-group ">
                            ...
                            <input type="text" class="form-control" id="inputEmail4" placeholder="title" name="title" required value="{{ $menus->title}}">
                        </div>                        
                        <hr>
                        <div class="form-group  ">
                            ...
                            <select name="type"  class="form-control">
                                ...
                                <option value="Food" {{ $menus->type == "Food"? 'selected' : '' }}>Food</option>
                                <option value="Hot Drink" {{ $menus->type == "Hot Drink" ? 'selected' : '' }}>Hot Drink</option>
                                <option value="Cold Drink" {{ $menus->type == "Cold Drink" ? 'selected' : '' }}>Cold Drink</option>
                            </select>
                        </div>
                        <hr>
                        <div class="form-group ">
                            <label for="exampleFormControlTextarea1">Menu Description</label>
                            <textarea name="description" class="form-control" id="exampleFormControlTextarea1" rows="3">{{ $menus->description}}</textarea>
                        </div>
                        <hr>
                        <div class="form-group ">
                            ...
                            <select name="status" id="inputState" class="form-control" >
                                ...
                                <option value="1" {{ $menus->status == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ $menus->status == 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                        <hr>
                        <div class="form-group">
                            ...
                            <img class="img-fluid img-thumbnail menu-img" src="/{{ $menus->image }}" />
                        </div>
                        <hr>
                        <button type="submit" class="pull-right btn btn-info">Update Menu</button>
                    </form>
                </div>
            </div>
        </div>
